...
int Bank::toTransfer(int currentID,int receiverID,int toTransferAmount){
    ...
    string toRecord = "From :"+username[currentID]+" to ->"+username[receiverID] +"Transfered"+ to_string(toTransferAmount)+"\n";
    outfile<<toRecord;
    outfile.close();
}
...
// အားနဲချက်များ
// record အားလုံး ပြန်ကြည့်ချင်ရင် file အားလုံးကို ဖွင့်ရမလို ဖြစ်နေတယ် to solve allRecord file တည်ဆောက်ပြီး သိမ်းထားရန်